...
export class CheckoutComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.shipping = this.subtotal > 250 ? 0.0 : this.subtotal == 0 ? 0.0 : 9.95;
  ...
  setBillingAddress() {
    ...
    if (this.checked) {
      this.checkoutService.checkout.BillingAddress =
        this.checkoutService.checkout.ShippingAddress;
      this.checkoutService.checkout.BillingCity =
        this.checkoutService.checkout.ShippingCity;
      this.checkoutService.checkout.BillingState =
        this.checkoutService.checkout.ShippingState;
      this.checkoutService.checkout.BillingZip =
        this.checkoutService.checkout.ShippingZip;
    } else {
      this.checkoutService.checkout.BillingAddress = "";
      this.checkoutService.checkout.BillingCity = "";
  ...
  setSelectedMonth(event) {
    ...
    if (parseInt(this.selectedMonth) >= parseInt(currentMonth)) {
      this.years = [
        ...
        parseInt(currentYear) + 2,
        parseInt(currentYear) + 3,
        parseInt(currentYear) + 4,
      ];
    } else {
      this.years = [
        parseInt(currentYear) + 1,
